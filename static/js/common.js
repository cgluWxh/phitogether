...
export const full = {
	...
	check(elem) {
		const canvas=$("canvas")
		if (!(elem instanceof HTMLElement)) elem = document.body;
		if(this.element == elem)return true;
		if(canvas.style.position=="fixed")return true;
		return false;
	},
	get onchange() {
...
export const urls = {
	zip: ['/static/js/zip.min.js'],
	jszip: ['/static/js/jszip.js'],
	browser: ['/static/js/Browser.js'],
	bitmap: ['/static/js/createImageBitmap.js'],
	blur: ['/static/js/stackblur-canvas.js'],
	md5: ['/static/js/md5-js.js'],
}

...
async function checkSupport() {
	...
	if (!await loadPlugin('ogg格式兼容', '/static/lib/oggmented-bundle.js', () => !oggCompatible && isUndefined('oggmented'))) return -4;
...
function adjustInfo() {
	...
	if(!inputName.value) {
		if (multi.edata.nowChartOptions)
			inputName.value = decodeURIComponent(multi.edata.nowChartOptions.downlink).replace(location.href,"").replace(/\/?static\/charts\//,"").replace(".zip","")
		else
			inputName.value = "";
...
//uploader
{
	...
	async function handleFile(data, total) {
		...
		adjustInfo();
		$("demo").disabled=false;
		if(multi.multiSelf.isMulti)multi.multiSelf.chartLoaded=true;
		if(multi.multiSelf.isMulti && (!multi.multiSelf.owner||multi.multiSelf.startNow))multi.multiSelf.loadChartSecond()
		/**
...
document.getElementById("demo").addEventListener("click", function(e) {
	...
	multi.edata.nowChartOptions={downlink:document.querySelector("#load-preset").selectedOptions[0].value}
	if(multi.multiSelf.isMulti)multi.multiSelf.chartLoaded=false;
	const xhr = new XMLHttpRequest();
	xhr.open("get", `${document.querySelector("#load-preset").selectedOptions[0].value}`, true); 
	xhr.responseType = 'arraybuffer';
	xhr.send();
	xhr.onprogress = progress => { //显示加载文件进度
		msgHandler.sendMessage(`加载文件中...`);
	};
	xhr.onload = (e) => {
		uploader.onload({ target: { result: e.target.response } }, { name: document.querySelector("#load-preset").selectedOptions[0].value });
	};
});
//qwq end
//hit start
const specialClick = {
	...
	func: [() => {
		...
		if(multi.multiSelf.isMulti)return;
		btnPlay.click();
		btnPlay.click();
	}, () => {
		showPoint.click();
...
document.addEventListener('DOMContentLoaded', async function qwq() {
	...
	let loadedNum = 0;
	let errorNum = 0;
	const pth = "/static/assets/###.png";
	const erc = str => str.includes('.') ? str : pth.replace('###', str);
	msgHandler.sendMessage('初始化...');
	multi.edata.charts=charts;
	multi.edata.stat=stat;
	...
	await Promise.all(Object.entries({
		...
		LevelOver0_v1: '/static/src/LevelOver0.ogg',
		LevelOver1_v1: '/static/src/LevelOver1.ogg',
		LevelOver2_v1: '/static/src/LevelOver2.ogg',
		LevelOver3_v1: '/static/src/LevelOver3.ogg',
...
function loop() {
	...
	ctx.fillText(`Phi Together v${thisVersion}i - Code by lchzh3473 and cgluWxh`, (canvas.width + canvasos.width) / 2 - lineScale * 0.1, canvas.height - lineScale * 0.2);
...
function qwqdraw1(now) {
	...
	if (!document.getElementById("closeAnim").checked)hitEvents1.anim(anim1, now);
...
function qwqdraw2() {
	...
	if(multi.multiSelf.isMulti)multi.multiSelf.uploadScore();
}
